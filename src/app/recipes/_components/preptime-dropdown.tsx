...
function getTriggerLabel(value: string | null) {
  const found = prepTimes.find((time) => time.value === value);
  return found ? found.label : null;
}

export const PrepTimeMenu = () => {
  const { updateURL, getParam } = useUrlState();
  const maxPT = getParam("pt");

  const handleSelectedValue = (value: string) => {
    updateURL({ pt: value });
  };

  const triggerLabel = getTriggerLabel(maxPT) ?? "Max Prep Time";
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-60 p-2" align="start">
        <DropdownMenuRadioGroup onValueChange={handleSelectedValue} value={maxPT}>
          {prepTimes.map((time) => (
